...
{	
	...
	if(contenido == ""){
		Swal.fire({
			html: '<h3>La historia no puede quedar vacía</h3>'	  
		});
	}else if(idPersonaje == "" || idPaisaje == ""){
		Swal.fire({
			html: '<h3>Por favor elige un persionaje y/o paisaje</h3>'	  
		});
	}else{
    $.ajax({
		...
		success: function(Resultado) {
			...
			Swal.fire({
				...
				window.location.href = "http://localhost/tt-gamificacion/gamiweb/tables/elegir_actividad.php?idHistoria="+objJson.id+"&"+"idGrupo="+idGrupo;
			});
		},
		error: function (data) {
			// body...
